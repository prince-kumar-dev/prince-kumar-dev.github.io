...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const pomodoroAlarmControls = document.querySelector(
  ...
  function loadDailyGoal() {
    const savedGoal = getLocalStorage("devHomeDailyGoal", {
      text: "",
      date: "",
    });
  ...
  function performSearch(queryOverride = null) {
    const query =
      queryOverride !== null ? queryOverride : searchInput.value.trim();
    if (query && searchEngineSelect) {
      const selectedOption =
        searchEngineSelect.options[searchEngineSelect.selectedIndex];
  ...
  function fetchSuggestions(query) {
    ...
    let fullSuggestUrl = "";
    if (suggestUrlBase.includes("google.com")) {
      fullSuggestUrl = `${suggestUrlBase}${encodeURIComponent(
        query
      )}&callback=${callbackName}`;
    } else if (suggestUrlBase.includes("duckduckgo.com")) {
      fullSuggestUrl = `${suggestUrlBase}${encodeURIComponent(
        query
      )}&callback=${callbackName}`;
    } else {
  ...
  if (searchEngineSelect)
    searchEngineSelect.addEventListener("change", clearSuggestions);

  // 5. Shortcuts
  let shortcuts = getLocalStorage("devHomeShortcuts", DEFAULT_SHORTCUTS);
  let draggedItem = null;

  // MODIFIED: renderShortcuts to add draggable attributes and simplify modal rendering logic
  function renderShortcuts() {
    if (!shortcutsContainer || !modalShortcutsList) return;

    // --- Render Main Shortcut Grid ---
    shortcutsContainer.innerHTML = ""; // Clear main grid
    shortcuts.forEach((shortcut, index) => {
      // Use index here for original position reference if needed later
      ...
      shortcutLink.setAttribute(
        "title",
        `Ctrl + ${index + 1}: Open ${shortcut.name}`
      ); // Keep keyboard shortcut hint

      // --- Drag & Drop Attributes ---
      shortcutLink.setAttribute("draggable", "true");
      shortcutLink.dataset.id = shortcut.url; // Use URL as a unique ID for finding data later

      shortcutLink.innerHTML = `
                    <i class="${shortcut.icon || "fas fa-link"}"></i>
                    <span>${shortcut.name}</span>
                `;
      shortcutsContainer.appendChild(shortcutLink);

      // Add drag event listeners directly here
      addDragEvents(shortcutLink);
    });

    // --- Render Modal Shortcut List (Simplified) ---
    renderModalShortcuts(); // Call separate function to render modal list
  }

  // NEW: Function specifically for rendering the modal list
  function renderModalShortcuts() {
    if (!modalShortcutsList) return;
    modalShortcutsList.innerHTML = ""; // Clear modal list
    shortcuts.forEach((shortcut, index) => {
      // Use the current 'shortcuts' array order
      const modalEntry = document.createElement("div");
      modalEntry.className = "shortcut-entry";
      // Ensure delete button uses the CURRENT index from the potentially reordered array
      modalEntry.innerHTML = `
            <span><i class="${shortcut.icon || "fas fa-link"}"></i> ${
        shortcut.name
      } (${shortcut.url.substring(0, 30)}...)</span>
            <button data-index="${index}" class="delete-shortcut-btn" title="Delete Shortcut">Delete</button>
        `;
      modalShortcutsList.appendChild(modalEntry);
    });

    // Re-attach delete listeners for the modal
    document
      .querySelectorAll("#modal-shortcuts-list .delete-shortcut-btn")
      .forEach((button) => {
        button.addEventListener("click", (e) => {
          ...
          deleteShortcut(indexToDelete); // deleteShortcut already saves and re-renders
        });
      });
  }
  ...
  function addShortcut(name, url, icon) {
    if (name && url) {
      ...
      shortcuts.push({ name, url, icon: icon || "fas fa-link" });
      setLocalStorage("devHomeShortcuts", shortcuts);
      renderShortcuts();
      ...
      alert("Please provide both a name and a URL for the shortcut.");
    }
  }
  // MODIFIED: deleteShortcut now only needs to re-render the modal list if called from modal
  function deleteShortcut(index) {
    if (index >= 0 && index < shortcuts.length) {
      shortcuts.splice(index, 1);
      setLocalStorage("devHomeShortcuts", shortcuts);
      renderShortcuts(); // Re-render both grid and modal list fully
    }
  }
  if (addShortcutBtn)
    addShortcutBtn.addEventListener("click", () => {
      if (newShortcutNameInput && newShortcutUrlInput && newShortcutIconInput) {
        addShortcut(
          newShortcutNameInput.value.trim(),
          newShortcutUrlInput.value.trim(),
          newShortcutIconInput.value.trim()
        );
      }
    });
  // NEW: Function to add drag event listeners to an element
  ...
  function handleDragStart(e) {
    // 'this' refers to the element the listener was attached to (the shortcutLink)
    draggedItem = this;
    this.classList.add("dragging");
    // Optional: You could set dataTransfer data if needed, but storing the element might be enough
  ...
  function handleDragEnd(e) {
    ...
    document.querySelectorAll(".shortcut-item.drag-over").forEach((item) => {
  ...
  function handleDragOver(e) {
    ...
    // Add drag-over class to the item being hovered over
  ...
  function handleDragEnter(e) {
    e.preventDefault(); // May be needed for some browsers/styles
    if (this !== draggedItem) {
      this.classList.add("drag-over");
    }
  }
  ...
  function handleDrop(e) {
    ...
    e.stopPropagation(); // Prevent drop event bubbling up if needed

    const dropTarget = this; // 'this' is the element where the drop occurred

    dropTarget.classList.remove("drag-over"); // Clean up target style

    // Ensure we are dropping on a different item and have a valid dragged item
    if (draggedItem && draggedItem !== dropTarget) {
      // --- 1. Reorder DOM Elements ---
      const parent = shortcutsContainer;
      const draggedRect = draggedItem.getBoundingClientRect();
      const targetRect = dropTarget.getBoundingClientRect();

      // Insert draggedItem before dropTarget
      // Check if dragging down or up to place correctly
      if (draggedRect.top < targetRect.top) {
        // Dragging Down: Insert after the target's current next sibling or at the end
      ...
      // --- 2. Update the 'shortcuts' Array Order ---
      const updatedShortcuts = [];
      const currentItems = parent.querySelectorAll(".shortcut-item");

      currentItems.forEach((itemElement) => {
        // Find the original shortcut data using the unique ID (URL)
        const shortcutData = shortcuts.find(
          (s) => s.url === itemElement.dataset.id
        );
        if (shortcutData) {
          updatedShortcuts.push(shortcutData);
        } else {
          console.warn(
            "Could not find shortcut data for element:",
            itemElement
          );
        }
      });
      ...
      // --- 4. Re-render the Modal List to reflect new order and update indices ---
      renderModalShortcuts(); // Only need to update the modal list's indices/order

      // --- 5. Optional: Re-assign keyboard shortcut titles (or do it in renderShortcuts) ---
      parent.querySelectorAll(".shortcut-item").forEach((item, index) => {
        item.setAttribute(
          "title",
          `Ctrl + ${index + 1}: Open ${item.querySelector("span").textContent}`
        );
      });
    }
  ...
  // --- Initialization ---
  // ... (load other things) ...
  renderShortcuts(); // Initial render
  // ... (rest of your init code) ..

  // 6. Task List
  function loadTasks() {
    tasks = getLocalStorage("devHomeTasks", []);
    renderTasks();
  }
  ...
  function saveAndRenderTasks() {
    setLocalStorage("devHomeTasks", tasks);
    renderTasks();
  }
  ...
  function updatePomodoroDisplay() {
    if (pomodoroTimeElement)
      pomodoroTimeElement.textContent = formatTime(pomodoroTimeLeft);
  }
  function updatePomodoroStatus(statusText = "") {
    ...
    if (statusText) {
      ...
      const modeText = pomodoroCurrentMode
        .replace(/([A-Z])/g, " $1")
        .replace(/^./, (c) => c.toUpperCase());
      pomodoroStatusElement.textContent = pomodoroIsRunning
        ? `${modeText} in progress...`
        : `Ready for ${modeText}`;
    }
  }
  ...
  function updatePauseResumeButton(isPaused) {
    if (pomodoroPauseAlarmBtn) {
      if (isPaused) {
        pomodoroPauseAlarmBtn.innerHTML =
          '<i class="fas fa-volume-up"></i> Resume Alarm';
        pomodoroPauseAlarmBtn.title = "Resume Alarm Sound";
      } else {
        pomodoroPauseAlarmBtn.innerHTML =
          '<i class="fas fa-volume-mute"></i> Pause Alarm';
      ...
      if (pomodoroTimeLeft <= 0) {
        ...
        if (pomodoroAlarmSound) {
          ...
          if (pomodoroAlarmControls)
            pomodoroAlarmControls.classList.add("visible"); // Show controls
          ...
          pomodoroAlarmSound.play().catch((e) => {
            ...
            alert(
              `${pomodoroCurrentMode.replace(/([A-Z])/g, " $1")} finished!`
            );
        ...
        const finishedModeText = pomodoroCurrentMode
          .replace(/([A-Z])/g, " $1")
          .replace(/^./, (c) => c.toUpperCase());
        ...
        pomodoroNextModeTimeoutId = setTimeout(
          () => {
            if (pomodoroIsAlarmPlaying) {
              // Only auto-switch if alarm wasn't manually stopped
          ...
          pomodoroAlarmSound ? pomodoroAlarmSound.duration * 1000 + 500 : 500
        ); // Wait for sound duration + buffer, or short delay
      }
    }, 1000);
  }
  ...
  if (pomodoroStopAlarmBtn) {
    pomodoroStopAlarmBtn.addEventListener("click", () => {
      ...
      let nextMode = "work";
      if (pomodoroCurrentMode === "work") nextMode = "shortBreak";
      const nextModeText = nextMode
        .replace(/([A-Z])/g, " $1")
        .replace(/^./, (c) => c.toUpperCase());
      updatePomodoroStatus(
        `${finishedModeText} Finished. Alarm stopped. Press Play for ${nextModeText}.`
      );
    });
  }
  ...
  function fetchWeather(options = {}) {
    const { coords, locationName } = options;
    let apiUrl = "";
    const units = "metric";
    if (!WEATHER_API_KEY || WEATHER_API_KEY === "YOUR_OPENWEATHERMAP_API_KEY") {
      if (weatherInfoElement)
        weatherInfoElement.innerHTML =
          '<i class="fas fa-exclamation-triangle"></i> Weather API key missing/invalid.';
    ...
    if (coords) {
      apiUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${coords.latitude}&lon=${coords.longitude}&appid=${WEATHER_API_KEY}&units=${units}`;
      ...
      if (weatherInfoElement)
        weatherInfoElement.innerHTML =
          '<i class="fas fa-map-marker-slash"></i> Location unavailable.';
      return;
    }
    fetch(apiUrl)
      .then((response) => {
      ...
      .then((data) => {
        ...
        } else if (weatherInfoElement) {
          weatherInfoElement.innerHTML = `<i class="fas fa-exclamation-circle"></i> Error: ${
      ...
      .catch((error) => {
        ...
        if (weatherInfoElement)
          weatherInfoElement.innerHTML = `<i class="fas fa-cloud-slash"></i> Could not fetch weather. ${
            error.message.includes("not found") ? "(City?)" : ""
          }`;
      });
  }
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          let fallbackMessage = "Could not get location. ";
          if (error.code === error.PERMISSION_DENIED) {
            fallbackMessage = "Location access denied. ";
          } else if (error.code === error.POSITION_UNAVAILABLE) {
            fallbackMessage = "Location info unavailable. ";
          } else if (error.code === error.TIMEOUT) {
            fallbackMessage = "Location request timed out. ";
          }
          const lastCity = getLocalStorage("devHomeWeatherLastCity", "London");
          if (weatherInfoElement)
            weatherInfoElement.innerHTML = `<i class="fas fa-info-circle"></i> ${fallbackMessage} Showing weather for ${lastCity}.`;
      ...
      const lastCity = getLocalStorage("devHomeWeatherLastCity", "London");
      if (weatherInfoElement)
        weatherInfoElement.innerHTML = `<i class="fas fa-map-marker-slash"></i> Geolocation not supported. Showing for ${lastCity}.`;
      fetchWeather({ locationName: lastCity });
    }
  }
  initWeather();
  ...
  window
    ...
    .addEventListener("change", (e) => {
      if (!getLocalStorage("devHomeTheme", null)) {
  ...
  if (settingsBtn)
    settingsBtn.addEventListener("click", () => {
      ...
      renderShortcuts();
      const currentBg = getLocalStorage("devHomeBackground", null);
      if (currentBg && bgTypeSelect) {
        bgTypeSelect.value = currentBg.type;
        renderBackgroundInputs(currentBg.type);
      } else {
        renderBackgroundInputs("color");
      }
    });
  if (closeModalBtn)
  ...
  function renderBackgroundInputs(type) {
    ...
    const defaultBgColor =
      getLocalStorage("devHomeTheme", "light") === "dark"
        ? "#1a1a1a"
        : "#f4f7f6";
    const currentBg = getLocalStorage("devHomeBackground", {
      type: "color",
      value: defaultBgColor,
    });
    switch (type) {
      case "color":
        inputHtml = `<input type="color" id="bg-color-input" value="${
          currentBg.type === "color" ? currentBg.value : defaultBgColor
        }">`;
        break;
      case "gradient":
        const gradValue =
          currentBg.type === "gradient"
            ? currentBg.value
            : ["#4dabf7", "#282c34"];
        inputHtml = `<label>Color 1: <input type="color" id="bg-gradient-color1" value="${
          gradValue[0] || "#4dabf7"
        }"></label><label>Color 2: <input type="color" id="bg-gradient-color2" value="${
          gradValue[1] || "#282c34"
        }"></label>`;
        break;
      case "image":
        const imgValue = currentBg.type === "image" ? currentBg.value : "";
        inputHtml = `<input type="url" id="bg-image-url" placeholder="Enter Image URL" value="${imgValue}">`;
        break;
    }
  ...
  function applyBackground(settings) {
    ...
    switch (settings.type) {
      case "color":
        ...
        break;
      case "gradient":
        document.body.style.backgroundImage = `linear-gradient(to bottom right, ${settings.value[0]}, ${settings.value[1]})`;
  ...
  if (bgTypeSelect)
    bgTypeSelect.addEventListener("change", (e) => {
      renderBackgroundInputs(e.target.value);
    });
  if (applyBgBtn)
    applyBgBtn.addEventListener("click", () => {
      ...
      switch (type) {
        case "color":
          value = document.getElementById("bg-color-input")?.value;
          break;
        case "gradient":
          value = [
            document.getElementById("bg-gradient-color1")?.value,
            document.getElementById("bg-gradient-color2")?.value,
          ];
          break;
        case "image":
      ...
      if (
        ...
        alert("Please provide a value for the background.");
      }
    });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.ctrlKey && e.key >= "1" && e.key <= "9" && !isInputFocused) {
      e.preventDefault();
      ...
      switch (e.key) {
        ...
        case "Enter":
          if (currentSuggestionIndex > -1) {
            e.preventDefault();
            items[currentSuggestionIndex].dispatchEvent(
  ...
  if (resetSettingsBtn)
    resetSettingsBtn.addEventListener("click", () => {
      if (
        confirm(
          "Are you sure you want to reset ALL settings (theme, name, shortcuts, tasks, goal, background, weather location)? This cannot be undone."
        )
      ) {
        localStorage.removeItem("devHomeTheme");
        localStorage.removeItem("devHomeUserName");
        localStorage.removeItem("devHomeShortcuts");
        localStorage.removeItem("devHomeTasks");
        localStorage.removeItem("devHomeDailyGoal");
        localStorage.removeItem("devHomeWeatherLastCity");
        localStorage.removeItem("devHomeBackground");
